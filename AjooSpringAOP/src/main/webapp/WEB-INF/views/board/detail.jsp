		...
		function getReplyList(){ // (...) 댓글 목록을 가져오는 함수 정의
			...
			$.ajax({ // jQuery의 Ajax 함수를 사용하여 서버에 비동기적으로 요청을 보냄.
				...
				success : function(result){ // 요청이 성공했을 때 실행할 콜백 함수 지정. 여기서는 서버로부터 받은 데이터를 이용하여 댓글 목록을 생성하고 화면에 표시
					var tableBody = $("#replyTable tbody"); // "replyTable" id를 가진 테이블의 tbody 요소를 선택하여 변수 tableBody에 저장. 이 요소는 댓글 목록을 표시할 테이블의 몸체 부분.
	//					tableBody.empty(); // * empty(); 제이쿼리 메소드로 삭제한 댓글들 남아있는 오류 해결
					tableBody.html(""); // * empty()가 아니더라도 html("")를 이용해서 비워줄 수 있음
										// * 테이블 몸체를 비움. 댓글 목록을 다시 가져와서 표시할 때 이전에 있던 댓글들을 모두 지우기 위해 사용
					var tr;
					var replyWriter;
					var replyContent;
					var rCreateDate;
					var btnArea;
					if(result.length > 0){ // (...)서버로부터 받은 데이터(result)의 길이가 0보다 큰 경우에만 댓글을 생성하고 화면에 표시함.
						for(var i in result){	
